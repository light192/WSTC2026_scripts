...
Для функциональных SSH/sudo-тестов LDAP-пользователей скрипт использует `sshpass`, если он установлен; иначе включает fallback через временный `SSH_ASKPASS`. Root SSH проверяется по ключу.
