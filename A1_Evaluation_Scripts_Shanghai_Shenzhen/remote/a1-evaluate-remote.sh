...
Options:
  --pause                  Pause between major precheck sections.
  --no-pause               Do not pause between sections.
...
ssh_cmd() {
  ...
  ssh -o BatchMode=yes -o ConnectTimeout="$A1_TIMEOUT" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "root@$ip" "$cmd" 2>&1
}
...
check_A1() {
  ...
  if should_run_criterion "A1.1"; then
    ...
    ok=1
    for h in "${HOSTS[@]}"; do
      echo "$out" | grep -A2 -- "--- $h ---" | grep -qx "$h" || ok=0
      echo "$out" | grep -A3 -- "--- $h ---" | grep -q "${h}.${A1_DOMAIN}" || ok=0
    done
  ...
  if should_run_criterion "A1.3"; then
    ...
    for h in "${HOSTS[@]}"; do
      ...
      echo "--- $h ---" | tee -a "$A1_DETAIL_LOG"; echo "$out" | tee -a "$A1_DETAIL_LOG"
  ...
  if should_run_criterion "A1.5"; then
    ...
    echo "--- sh-gw-a1 ---"; echo "$out1"; echo "--- sz-gw-a1 ---"; echo "$out2" | tee -a "$A1_DETAIL_LOG"
...
check_A2() {
  ...
  if should_run_criterion "A2.2"; then
    ...
    for n in "${!A_REC[@]}"; do
      out="$(ssh_cmd sz-client-a1 "dig @${A1_DNS_IP} +short ${n}.${A1_DOMAIN} A")"; echo "$n A -> $out" | tee -a "$A1_DETAIL_LOG"; echo "$out" | grep -qx "${A_REC[$n]}" || ok=0
    done
  ...
  if should_run_criterion "A2.3"; then
    ...
    for n in "${!AAAA_REC[@]}"; do
      out="$(ssh_cmd sz-client-a1 "dig @${A1_DNS_IP} +short ${n}.${A1_DOMAIN} AAAA")"; echo "$n AAAA -> $out" | tee -a "$A1_DETAIL_LOG"; echo "$out" | grep -qx "${AAAA_REC[$n]}" || ok=0
    done
  ...
  if should_run_criterion "A2.4"; then
    ...
    ok=1
    for n in "${names[@]}"; do
      out="$(ssh_cmd sz-client-a1 "dig @${A1_DNS_IP} +short ${n}.${A1_DOMAIN} A")"; echo "$n -> $out" | tee -a "$A1_DETAIL_LOG"; [ -n "$out" ] || ok=0
    done
  ...
  if should_run_criterion "A2.5"; then
    ...
    for ip in 10.11.10.1 198.51.100.10 198.51.100.20 10.11.20.1 10.11.30.1 10.11.40.1 10.11.10.20 10.11.20.20 10.11.30.10 10.11.40.10 10.11.40.20; do
      out="$(ssh_cmd sz-client-a1 "dig @${A1_DNS_IP} +short -x $ip")"; echo "$ip PTR -> $out" | tee -a "$A1_DETAIL_LOG"; echo "$out" | grep -q "${A1_DOMAIN}" || ok=0
    done
  ...
  if should_run_criterion "A2.6"; then
    ...
    for n in dns ldap ca files www; do
      out="$(ssh_cmd sz-client-a1 "dig @${A1_DNS_IP} +short ${n}.${A1_DOMAIN} CNAME")"; echo "$n CNAME -> $out" | tee -a "$A1_DETAIL_LOG"; [ -n "$out" ] || ok=0
    done
  ...
  if should_run_criterion "A2.8"; then
    ...
    echo "internal recursion: $out1"; echo "WAN-source recursion: $out2" | tee -a "$A1_DETAIL_LOG"
  ...
  if should_run_criterion "A2.9"; then
    ...
    for h in sh-client-a1 sz-client-a1 files-a1 web-a1; do
      out="$(ssh_cmd "$h" "grep -R '${A1_DNS_IP}' /etc/resolv.conf /etc/systemd/resolved.conf /etc/systemd/network 2>/dev/null || true")"; echo "--- $h --- $out" | tee -a "$A1_DETAIL_LOG"; [ -n "$out" ] || ok=0
    done
...
check_A3() {
  ...
  if should_run_criterion "A3.2"; then
    ...
    for h in sz-client-a1 sh-client-a1 files-a1 web-a1; do
      out="$(ssh_cmd "$h" 'openssl verify -CApath /etc/ssl/certs /opt/grading/a1/ca.pem 2>/dev/null || grep -R "Orion A1 Root CA" /etc/ssl/certs /usr/local/share/ca-certificates 2>/dev/null || true')"; echo "--- $h --- $out" | tee -a "$A1_DETAIL_LOG"; [ -n "$out" ] || ok=0
    done
...
check_A5() {
  ...
  if should_run_criterion "A5.4"; then
    ...
    echo "$out1"; echo "$out2" | tee -a "$A1_DETAIL_LOG"
