#!/usr/bin/env bash
# Install the ops-a6 expert public key locally on a target VM.
...
if [ "$ALLOW_ROOT" = 1 ]; then
  ...
  printf '%s\n' 'PermitRootLogin prohibit-password' 'PubkeyAuthentication yes' > /etc/ssh/sshd_config.d/99-a6-marking.conf
  sshd -t && systemctl reload ssh
fi
echo "A6 expert key installed on $(hostname -s)"
