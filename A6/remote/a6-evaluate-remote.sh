...
echo A6_OK" ;;
    ...
    A1.08) echo "failed=0; service=\$(ssh root@10.76.40.20 'systemctl show kea-dhcp4-server -p ExecStart --value') || failed=1; cfg=\$(sed -n 's/.*[[:space:]]-c[[:space:]]\\([^ ;}]*\\).*/\\1/p' <<<\"\$service\" | head -n1); [ -n \"\$cfg\" ] || cfg=/etc/kea/kea-dhcp4.conf; ssh root@10.76.40.20 \"ls -l '\$cfg'\" || failed=1; ssh root@10.76.40.20 \"kea-dhcp4 -t '\$cfg'\" || failed=1; py=\$(mktemp); trap 'rm -f \"\$py\"' EXIT INT TERM; cat >\"\$py\" <<'PY'
import json, re, sys
...
def check(label, actual, expected):
    ...
    if actual == expected:
        ...
        print('[FAIL]', label, 'expected:', expected)
        failed = True
...
try:
    ...
    dhcp = json.loads(text)['Dhcp4']
    subnets = {item['subnet']: item for item in dhcp['subnet4']}
    sh = subnets['10.76.10.0/24']
    sz = subnets['10.76.20.0/24']
    check('Shanghai pool', sh['pools'][0]['pool'].replace(' ', ''), '10.76.10.110-10.76.10.149')
    check('Shenzhen pool', sz['pools'][0]['pool'].replace(' ', ''), '10.76.20.110-10.76.20.149')
    sh_res = [r for r in sh.get('reservations', []) if r.get('ip-address') == '10.76.10.100' and r.get('hw-address')]
    sz_res = [r for r in sz.get('reservations', []) if r.get('ip-address') == '10.76.20.100' and r.get('hw-address')]
    check('Shanghai reservation 10.76.10.100', bool(sh_res), True)
    check('Shenzhen reservation 10.76.20.100', bool(sz_res), True)
    for name, subnet, router in [('Shanghai', sh, '10.76.10.1'), ('Shenzhen', sz, '10.76.20.1')]:
        options = {option['name']: option['data'] for option in subnet['option-data']}
        check(name + ' router', options.get('routers'), router)
        dns = ', '.join(part.strip() for part in options.get('domain-name-servers', '').split(','))
        check(name + ' DNS', dns, '10.76.40.10, 10.76.40.20')
        check(name + ' domain', options.get('domain-name'), 'nova.a6.test')
except Exception as error:
    print('[FAIL] unable to read/parse Kea configuration:', error)
    failed = True
raise SystemExit(1 if failed else 0)
PY
ssh root@10.76.40.20 \"python3 - '\$cfg'\" <\"\$py\" || failed=1; rm -f \"\$py\"; trap - EXIT INT TERM; [ \$failed -eq 0 ] && echo A6_OK" ;;
