...
echo A6_OK" ;;
    ...
    C1.01) echo "ssh root@10.76.40.10 'ldif=\$(slapcat -b dc=nova,dc=a6,dc=test) || exit 1; printf \"%s\\n\" \"\$ldif\" | grep -Ei \"^(dn|ou|uid|uidNumber|gidNumber|cn|member|memberUid):\"; record(){ awk -v key=\"\$1\" \"BEGIN { RS=\\\"\\\" } index(tolower(\\\$0),tolower(key)) { print; found=1 } END { exit !found }\" <<<\"\$ldif\"; }; for ou in People Groups Services; do grep -qi \"^dn: ou=\$ou,dc=nova,dc=a6,dc=test\$\" <<<\"\$ldif\" || exit 1; done; m=\$(record \"uid: maya\") || exit 1; t=\$(record \"uid: timur\") || exit 1; l=\$(record \"cn: linuxusers\") || exit 1; f=\$(record \"cn: filewriters\") || exit 1; w=\$(record \"cn: webadmins\") || exit 1; grep -q \"uidNumber: 8601\" <<<\"\$m\" && grep -q \"gidNumber: 7600\" <<<\"\$m\" && grep -q \"uidNumber: 8602\" <<<\"\$t\" && grep -q \"gidNumber: 7600\" <<<\"\$t\" && grep -q \"gidNumber: 7600\" <<<\"\$l\" && grep -qi maya <<<\"\$l\" && grep -qi timur <<<\"\$l\" && grep -q \"gidNumber: 7601\" <<<\"\$f\" && grep -qi maya <<<\"\$f\" && ! grep -qi timur <<<\"\$f\" && grep -q \"gidNumber: 7610\" <<<\"\$w\" && grep -qi maya <<<\"\$w\" && ! grep -qi timur <<<\"\$w\"' && echo A6_OK" ;;
