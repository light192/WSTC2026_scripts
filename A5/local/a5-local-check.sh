...
case "$HN" in
  sh-gw-a5|sz-gw-a5)
    ...
    capture A5.3.02 0.25 dhcp-relay "ps aux | grep -i '[d]hcrelay\|[d]hcp-relay'; cat /etc/default/isc-dhcp-relay 2>/dev/null"
    ;;
  idm-a5)
    ...
    capture A5.6.01 0.25 chrony "chronyc tracking; grep -E '^allow|^local stratum' /etc/chrony/chrony.conf"
  ...
  sh-client-a5|sz-client-a5)
    ...
    ;;
  *) warn A5.1.01 0.25 "неизвестный hostname $HN" ;;
esac
write_summary
