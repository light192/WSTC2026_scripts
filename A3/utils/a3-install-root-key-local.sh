#!/usr/bin/env bash
# Install the admin-a3 expert public key locally on a target VM.
...
if [ "$ALLOW_ROOT" = 1 ]; then
  ...
  printf '%s\n' 'PermitRootLogin prohibit-password' 'PubkeyAuthentication yes' > /etc/ssh/sshd_config.d/99-a3-marking.conf
  sshd -t && systemctl reload ssh
fi
echo "A3 expert key installed on $(hostname -s)"
