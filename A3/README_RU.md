...
Фактический вывод появляется построчно во время выполнения. Все SSH-команды
принудительно работают в `BatchMode` с коротким timeout: недоступный узел даёт
быстрый FAIL и не удерживает весь evaluator на password/host-key prompt.
