...
Start state: restore every device/VM to the `D1-CLEAN-PDF-MATCHED` snapshot
built from `Clean_Baseline_Scripts`.
...
Use `validation/DAY2_EXPECTED_SYMPTOMS.md` to verify that the intended
symptoms are visible. All faults in this package use addressing that matches
`D1_Day2_Competitor_Task_EN_styled.pdf` directly — no IP translation step is
needed, and the Linux SSH-block fault (T05) is saved with
`netfilter-persistent` so it survives a VM reboot.
