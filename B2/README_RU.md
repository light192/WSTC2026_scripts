...
```powershell
.\hosts\check-BJ-SRV01.ps1 -StartFromAspect C1.05
```
...
ожидаемый результат, реально выполненная команда, отфильтрованный фактический
вывод и решение `PASS`, `FAIL` или `WARN`.
...
Некоторые аспекты из схемы требуют свидетельств с двух клиентов. Они включены
в оба соответствующих локальных скрипта. Баллы таких аспектов между отчетами
хостов не складываются: это две части доказательства одного аспекта.
...
- `criteria\b2_criteria_map.tsv` - 68 аспектов актуальной схемы с host mapping;
